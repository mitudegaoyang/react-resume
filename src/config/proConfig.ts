const BASECONFIG = [
  {
    ...
    href: "",
    description: [
      "中诚信国际智慧分析平台专为金融机构、企业和投资者设计，提供深度数据挖掘和专业信用评级服务，助力复杂金融环境下的明智决策。平台涵盖企业融资、政府公共融资、金融机构、结构融资、非标产品、主权及熊猫债评级等领域，集成首页概览、主体模块、区域模块和工具模块四大核心功能。",
      "首页展示个性化信息，主体模块深入分析企业信用状况，包括概览、股权结构、信用评级、经营和财务表现等。区域模块分析各地行业分布、经济和财政状况，揭示主体情况、政府和环境评分及研究报告。工具模块集成五个子功能——主体信用评分、政府信用评分、区域环境评分、同行业对比和区域数据对比，支持多角度数据分析和精准决策。",
